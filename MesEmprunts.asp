...
query ="ps_sel_empr_client "&Session("IdUser")&", 0"
...
<body>
	...
	<div class="container">
		...
		<form name="srchForm" id="srchForm" method="post" action="">
			<div class="rechDiv" border="0">
				...
				<div class="div-retard">
					<input type="checkbox" name="retard" id="retard" value="retard" onclick="showData()"><label
						style="padding-left: 5px;" for="retard">Livres en
						retard</label>
				</div>
				<select id="sel_col" onchange="showData()">
					<option value="CodeAbonnement" id="abonn" selected>par Abonnement</option>
					<option value="DesignationLivre" id="livre">par Livre</option>
					<option value="DateEmprunt" id="date">par Date</option>
				</select>
			</div>
		</form>
	...
	<script>
		function showData() {
			...
			//si je choisis les livres en retard
			var retardCb = document.getElementById('retard');
			if (retardCb.checked == true)
				retard = 1;
			else
				retard = 0;
			...
			xmlhttp.open("GET", "Recherche.asp?historique=false&filtre=" + filtre + "&valeur=" + valeur + "&retard=" +
				retard + "&date1=" + date1 + "&date2=" + date2, true);
			xmlhttp.send();
		}
	</script>
